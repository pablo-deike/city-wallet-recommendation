...
export default function MerchantView({ onBack }) {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', position: 'relative', display: 'flex', flexDirection: 'column', background: '#f5f7fb' }}>
          ...
          {sliders && (
            <div style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                ...
                <button onClick={() => setShowModal(true)} style={{ background: C.accentSoft, border: `1px solid ${C.border}`, borderRadius: 10, padding: '6px 12px', fontSize: 12, fontWeight: 700, color: C.text, cursor: 'pointer' }}>
                  Edit Rule
                </button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: C.elevated, borderRadius: 12, border: `1px solid ${C.border}` }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>{label}</div>
                      <div style={{ fontSize: 13, fontWeight: 800, color: C.text, marginTop: 1 }}>{value}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showModal && sliders && (
        <div onClick={e => e.target === e.currentTarget && setShowModal(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(15,23,42,0.35)', display: 'flex', alignItems: 'flex-end', zIndex: 200 }}>
          <div style={{ width: '100%', background: C.surface, borderRadius: '24px 24px 0 0', padding: '0 24px 36px', border: `1px solid ${C.border}`, borderBottom: 'none' }}>
            ...
            {RULE_FIELDS.map(({ key, label, unit, min, max }) => (
              <div key={key} style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 800, color: '#b45309' }}>{sliders[key]}{unit}</span>
                </div>
                <input
                  type="range"
                  min={min}
            ...
            <button onClick={() => handleSave(sliders)} style={{ width: '100%', background: C.accent, color: 'white', border: 'none', borderRadius: 14, padding: 15, fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 4, boxShadow: '0 4px 16px rgba(91,154,245,0.3)' }}>
              Done
            </button>
          </div>
        </div>
